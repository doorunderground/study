...
// 배열의 최소, 최대, 평균, 분산, 표준편차를 위한 구하고 출력
void getArrayStatistics(int* array, int size) 
{
    int max = array[0];
    int min = array[0];
    double mean = 0, var = 0, std_var = 0, sample_mean = 0, sum =0;
    ...
    for (int i = 0; i < size; i++)
    {
        sample_mean += ((array[i] - mean) * (array[i] - mean));
    }
    var = sample_mean / size; //분산
    std_var = sqrt(var); //루트
    printf("\nmin (%3d), max (%3d), sum (%5.2lf), average (%5.2lf), variance (%5.2lf), standard deviation (%5.2lf)\n", min, max, sum, mean, var, std_var);
}
